...
#                        newW[tmp_row] = np.maximum(self.W[row1], self.W[row2])
                        ...
                        if (not modifiedIsOverlap(newV, newW, pairewise_maxb[i, 0].astype(np.int64), newClassId)) and (((newW[pairewise_maxb[i, 0].astype(np.int64)] - newV[pairewise_maxb[i, 0].astype(np.int64)]) <= self.teta).all() == True):
                            self.V = newV
                            self.W = newW
